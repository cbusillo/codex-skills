...
| Executable helper tests | Helper scripts listed in `scripts/validate-skills.sh` are executable test entrypoints. Any helper that imports or declares a `pytest` dependency must remain runnable as `uv run path/to/helper.py` by defining a module-level `if __name__ == '__main__': raise SystemExit(pytest.main([__file__]))` guard. Pytest use is detected from static absolute `pytest` imports, calls to a function named `import_module` or `__import__` with a literal `pytest` module name, and declared PEP 723 `pytest` dependencies. Indirection through an aliased importer or a computed module name is outside this bounded syntactic scope. The direct pytest exit must be the guard's first executable statement after optional docstrings or imports, and no direct module-level `raise SystemExit`, `sys.exit`, or `os._exit` may precede the guard. This is a bounded syntactic reachability guarantee, not full program-level control-flow or import-side-effect analysis. The skip list contains non-test CLIs and fixtures and is outside this executable-test contract. | Add or update the helper-test guard in the same PR as the helper itself. | `scripts/validate_execution_environment.py` parses the declared helper list, reads PEP 723 dependency metadata and literal dynamic-import calls, rejects missing, unreadable, or unparseable helpers, and uses AST validation to enforce the documented reachable `pytest.main` entrypoint shape for pytest-using helpers. |
