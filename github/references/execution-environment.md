...
| Executable helper tests | Helper scripts listed in `scripts/validate-skills.sh` are executable test entrypoints. Any helper that imports or declares a `pytest` dependency must remain runnable as `uv run path/to/helper.py` by defining a module-level `if __name__ == '__main__': raise SystemExit(pytest.main([__file__]))` guard. The direct pytest exit must be the guard's first executable statement after optional docstrings or imports, so an earlier exit or unreachable nested call cannot silently pass. The skip list contains non-test CLIs and fixtures and is outside this executable-test contract. | Add or update the helper-test guard in the same PR as the helper itself. | `scripts/validate_execution_environment.py` parses the declared helper list, reads PEP 723 dependency metadata, rejects missing, unreadable, or unparseable helpers, and uses AST validation to enforce a directly reachable `pytest.main` entrypoint for pytest-using helpers. |
