...
| Skill installer GitHub refs | Branches, tags, explicit `refs/...` names, and full 40-character commit SHAs are resolved through GitHub to one immutable commit before skill content is fetched. Bare branch/tag collisions, short SHAs, repository identity drift, ambiguous tree/blob URLs, and symbolic-link content fail closed. | Callers may continue selecting mutable names such as `main`; each install records the requested ref and the resolved full SHA. Private repositories require `GITHUB_TOKEN` or `GH_TOKEN` for resolution. | Archive download and git fallback receive only the resolved SHA, git verifies `HEAD`, git metadata is excluded, and focused offline tests cover mutable refs, SHA inputs, fallback consistency, annotated tags, ambiguity, drift rejection, transport parity, and public-safe provenance output. |
