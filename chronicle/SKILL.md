...
1. Only use this skill if memories are available in this rollout. This will
...
- When you need recent screen history instead of only the latest frame, search
  the OCR sidecars first. Use `rg` over `*.ocr.jsonl` to find relevant terms or
...
- Screen recordings may not always be up-to-date. You must use the `date`
