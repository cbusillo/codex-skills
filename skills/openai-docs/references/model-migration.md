...
Treat returned guide URLs as opaque: fetch those exact URLs without deriving, substituting, or appending a model query. Use an available official documentation or first-party-domain capability to open and read the relevant official page. Treat non-2xx, empty, whitespace-only, or non-substantive guide responses as unavailable. Retry the exact guide URL once when its response contains only a title or no substantive body, then try current official documentation search and fetch before using disclosed bundled fallbacks.
